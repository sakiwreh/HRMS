...
public class TravelDocumentServiceImpl implements ITravelDocumentService {
    ...
    @Override
    public TravelDocumentResponseDto upload(Long travelId, Long uploaderId, Long uploadedForId, DocType type,
        ...
        Employee uploadedBy = employeeRepository.findById(uploaderId)
                .orElseThrow(() -> new ResourceNotFoundException("Employee doesn't exist."));

        // Only participants or HR (plan creator) can upload documents
        ...
        if (uploadedForId != null) {
            doc.setUploadedFor(employeeRepository.findById(uploadedForId).orElseThrow(
                    () -> new ResourceNotFoundException("Employee for which this is uploaded doesn't exist.")));
        }
        doc.setDocType(type);
    ...
    public List<TravelDocumentResponseDto> getByTravelId(Long travelId, Long empId, String role) throws IOException {
        ...
        List<TravelDocument> docs = travelDocumentRepository.findByTravelPlanId(travelId);
    ...
    public void delete(Long docId, Long requesterId, String role) throws IOException {
        ...
        TravelDocument doc = travelDocumentRepository.findById(docId)
                .orElseThrow(() -> new ResourceNotFoundException("Document doesn't exist"));

        // Only the original uploader or HR can delete a document
    ...
    private TravelDocumentResponseDto toDto(TravelDocument doc) {
        TravelDocumentResponseDto dto = modelMapper.map(doc, TravelDocumentResponseDto.class);
        ...
        if (doc.getUploadedFor() != null) {
            dto.setUploadedForId(doc.getUploadedFor().getId());
            dto.setUploadedForName(doc.getUploadedFor().getFirstName() + " " + doc.getUploadedFor().getLastName());
        }
        return dto;
    }
}